    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or (Choice > 3 and Choice != 7):
                Choice = int(input("Choose move option to use from queue (1 to 3), 7 to replace a Kotla, or 9 to take the offer: "))
                if Choice == 9:
            ...
            if Choice == 7:
                ...
                while not hasChanged:
                    while not SquareIsValid:
                        StartSquareReference = self.__GetSquareReference("containing the piece to change to a Kotla")
                        SquareIsValid = self.__CheckSquareIsValid(StartSquareReference, True)

                    index = self.__GetIndexOfSquare(StartSquareReference)
                    square = self._Board[index]

                    if not square.ContainsKotla():
                        if self._CurrentPlayer.SameAs(self._Players[0]):
                            square = Kotla(self._Players[0], "K")
                            self._Board[index] = square
                        else:
                            square = Kotla(self._Players[1], "k")
                            self._Board[index] = square
                        hasChanged = True
                    else:
                        SquareIsValid = False
                        print("This square already contains a Kotla")
            else:
                while not SquareIsValid:
